...
loaded = np.load('../data/val_org_split_UserAwkwardness.npz',
                 allow_pickle=True)
val_x_sequences, val_y_sequences, val_sessions = prepare_seqs(loaded)

loaded = np.load('../data/train_org_split_UserAwkwardness.npz',
                 allow_pickle=True)
...
patience = 3
feature_dim = 90
model_name = 'multimodal_LSTM_' + str(units) + '_UA.keras'
